...
if($invo!=''){
    $sql = "INSERT INTO pd_purchase_list (invoice_no,name,qty,date,product_id,sell,type,user_id,stock_id,amount,unit,unit_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
    $re = $db->prepare($sql);
...
if ($type == 'Order') {
    if ($id2 == 1) {
        ...
        $result = $db->prepare("SELECT SUM(amount) FROM pd_purchases_list WHERE invoice_no = :id AND approve != '5'");
        ...
        */
        ...
        $updateAmount = $db->prepare("UPDATE pd_purchases SET amount = :amount WHERE invoice_no = :invoice_no");
