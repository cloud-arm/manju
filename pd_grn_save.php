...
$result = query("SELECT * FROM supplier WHERE id='$sup' ");
...
// $result = query("SELECT * FROM bank WHERE id='$bank' ");
// for ($i = 0; $row = $result->fetch(); $i++) {
//     $bank_name = $row['name'];
// }

$result = query("SELECT sum(amount),sum(discount) FROM pd_purchase_list WHERE invoice_no='$invo' AND approve !='5' ");
for ($i = 0; $row = $result->fetch(); $i++) {
...
if ($invo != '') {
    $sql = "INSERT INTO pd_purchase (invoice_no,amount,remarks,date,supplier_id,supplier_name,supplier_invoice,pay_type,pay_amount,discount,type,user_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
    $re = $db->prepare($sql);
    ...
    if ($type == 'GRN') {
        ...
        $result = query("SELECT * FROM pd_purchase_list WHERE invoice_no = '$invo' ");
        for ($i = 0; $row = $result->fetch(); $i++) {
            $p_id = $row['product_id'];
            $name = $row['name'];
            $qty = $row['qty'];
            ...
            $qty_blc = 0;
            $re = query("SELECT * FROM materials WHERE id = '$p_id' ");
            ...
            */
            ...
            $sql = "UPDATE pd_purchase SET action='5' WHERE invoice_no=? AND type='Order'";
            $ql = $db->prepare($sql);
            $ql->execute(array($invo));

           // stock_log('GRN',$invo, $p_id,$qty,$row['id']);
            $sql = "INSERT INTO inventory (name,type,qty,date,sell,cost,material_id,balance,invoice_no) VALUES (?,?,?,?,?,?,?,?,?)";
            $ql = $db->prepare($sql);
            $ql->execute(array( $name, 'in', $qty, $date, $sell, $cost,$p_id,$st_qty+$qty,$invo));

            $qty_blc = 0;
            $con = 0;
            $re = query("SELECT * FROM stock ");
            for ($k = 0; $r = $re->fetch(); $k++) {
            ...
            $r1 = select("pd_purchase_list", "invoice_no = '$invo' AND approve = '1' ");
            $sql = "UPDATE pd_purchase_list SET type = 'GRN' WHERE invoice_no = '$invo' AND approve = '1'";
            $stmt = query($sql);
        ...
        if ($pay_type == 'Cash') {
            ...
            $de_blc = 0;
            $blc = 0;
            $re = query("SELECT * FROM cash WHERE id = $cr_id ");
            for ($k = 0; $r = $re->fetch(); $k++) {
                $blc = $r['amount'];
                $cr_name = $r['name'];
            }
