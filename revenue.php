...
<body class="hold-transition skin-yellow skin-orange sidebar-mini">
    ...
    for ($i = 0; $row = $result->fetch(); $i++) {
        ...
        if ($insurance_diff <= 30 && $insurance_diff <= 0) {
            $insurance_class = 'bg-red'; // Less than 1 month
        } elseif ($insurance_diff <= 60 && $insurance_diff > 30) {
            $insurance_class = 'bg-warning'; // 1-2 months
        } elseif ($licence_diff > 0) {
            $licence_class = 'bg-green'; // Expired
        }
        else {
            $insurance_class = ''; // Normal
        }

        // Determine CSS class for licence date
        if ($licence_diff <= 30 && $licence_diff <= 0) {
            $licence_class = 'bg-red'; // Less than 1 month
        } elseif ($licence_diff <= 60 && $licence_diff > 30) {
            $licence_class = 'bg-warning'; // 1-2 months
        } elseif ($licence_diff > 0) {
            $licence_class = 'bg-green'; // valit
        }
         else {
            $licence_class = ''; // Normal
        } 
    ?>
        <tr>
...
<td>
                        <?php if ($user_level == 1): ?>
                        <button onclick="click_open('edit', <?php echo $row['id']; ?>)" class="btn btn-sm btn-warning">Revenue</button>
                        <?php endif; ?>
                    </td>
                </tr>
