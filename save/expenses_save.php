...
if ($unit == 1) {
    ...
    if($type_name == 'Repaire'){
        $util_gate_pass = $_POST['tool_name'];
    }
    ...
    if ($pay_type == 'chq') {
        ...
        // $q = $db->prepare($sql);
        // $q->execute(array($amount, $amount, $pay_type, $date, $invo, 0, 0, 0, '', $chq_no, $acc_name, $acc, $chq_date, '', 3, 1));
    }

    $sql = "INSERT INTO expenses_records (date,type_id,type,invoice_no,acc_id,acc_name,comment,amount,user,util_id,util_name,util_date,util_invoice,util_bill_amount,util_balance,util_forward_balance,pay_type,chq_no,chq_date,tool_name) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    $q = $db->prepare($sql);
    $q->execute(array($date, $type, $type_name, $invo, $acc, $acc_name, $comment, $amount, $ui, $util_id, $util_name, $util_date, $util_invo, $util_amount, $util_blc, $util_fw_blc, $pay_type, $chq_no, $chq_date,$util_gate_pass));
}
